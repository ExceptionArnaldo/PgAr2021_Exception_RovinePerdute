...
public class Team {
    ...
    public void dijkstra(Citta citta_partenza) {
        ...
        distanza[citta_partenza.getId()] = 0; // la distanza per arrivare alla sorgente è 0
        ...
        for (int i = 0; i < numero_citta; i++) {
            ...
            for (int j = 0; j < numero_citta; j++) {                                                            // condizione:
                if (!visitato[j] &&                                                                             // 1. la citta non è stata ancora visitata
                        ...
                        citta.get(id_attuale).getPercorsi().get(j) != 0 &&                                      // 3. la distanza non sia 0
    ...
    public int distanzaPiuBreve(double[] distanza, boolean[] visitato) {
        ...
        for (int i = 0; i < distanza.length; i++) { // scorre l'array di distanze
    ...
    public void setPercorsoMinimo(int[] citta_precedenti) {
        ...
        int indice = citta_precedenti.length - 1; // parte dalle rovine perdute

        percorso_minimo.push(Citta.getCittaById(indice));

        while (citta_precedenti[indice] != -1) { // fa fichè non arriva alla sorgente
